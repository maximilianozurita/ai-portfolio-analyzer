<script>
	...
	function buildOption(stocks, marketPrices, mode) {
	...
	onMount(() => {
		...
		chart.setOption(buildOption(stocks, marketPrices, mode));
		window.addEventListener('resize', () => chart.resize());
	});

	$: if (chart) chart.setOption(buildOption(stocks, marketPrices, mode));
