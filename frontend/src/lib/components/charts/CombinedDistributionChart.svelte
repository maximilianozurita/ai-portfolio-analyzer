<script>
	...
	export let stocks = [];
	export let bonds = [];
	export let marketPricesStocks = {};
	export let marketPricesBonds = {};
	...
	$: hasMarket =
		Object.values(marketPricesStocks).some(p => p?.current_value != null) ||
		Object.values(marketPricesBonds).some(p => p?.current_value != null);

	function buildOption(stocks, bonds, marketPricesStocks, marketPricesBonds, mode) {
		const stockItems = stocks.map(s => {
			const mp = marketPricesStocks[s.ticket_code];
			const value = mode === 'market' && mp?.current_value != null
				? Number(mp.current_value.toFixed(2))
				: Number((s.ppc * s.quantity).toFixed(2));
			return { name: s.ticket_code, value };
		});

		const bondItems = bonds.map(h => {
			const mp = marketPricesBonds[h.bond_code];
			const value = mode === 'market' && mp?.current_value != null
				? Number(mp.current_value.toFixed(2))
	...
	onMount(() => {
		...
		chart.setOption(buildOption(stocks, bonds, marketPricesStocks, marketPricesBonds, mode));
		window.addEventListener('resize', () => chart.resize());
	});

	$: if (chart) chart.setOption(buildOption(stocks, bonds, marketPricesStocks, marketPricesBonds, mode));
